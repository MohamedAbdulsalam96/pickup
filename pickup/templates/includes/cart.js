...
$.extend(shopping_cart, {
	...
	bind_pickup_slot: function() {
		$(".cart-pickup").on("change", "#pickup_slots", function() {
			...
			$('input[data-address-name="'+$("option:selected", this).attr("data-address")+'"]').prop('checked', true);
			...
			return frappe.call({
					...
					freeze: true,
					args: {
						slot_name: this.value,
						address_name: $("option:selected", this).attr("data-address")
					},
					callback: function(r) {
						if(!r.exc) {
...
frappe.ready(function() {
	...
	$('#pickup_slots').val(pickup_slot).change();
